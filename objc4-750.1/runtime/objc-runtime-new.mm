                              ...
                              size_t *outAllocatedSize = nil)
{
    ...
    assert(cls->isRealized()); // assert值为false，则退出程序执行；这里是看class是否已经实现，未实现值为0

    // Read class's info bits all at once for performance\
    // 判断class或者superclass是否有构造函数
    bool hasCxxCtor = cls->hasCxxCtor();
    // 判断class或者superclass是否有析构函数
    bool hasCxxDtor = cls->hasCxxDtor();
    // 是对isa类型的区分，如果一个class和他父类的实例不能使用isa_t 的isa指针的话，fast就为false；objc2.0中大部分类都是支持的。
    bool fast = cls->canAllocNonpointer();
    ...
    if (!zone  &&  fast) {
        ...
        if (zone) {
            // 当zone存在的时候，在zone中分配一块size大小的内存空间
...
void *objc_destructInstance(id obj) 
{
    if (obj) {
        ...
        bool assoc = obj->hasAssociatedObjects();   // 是否有关联引用
